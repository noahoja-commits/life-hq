...
import { Rocket, Plus, Trash2, ExternalLink, ArrowRight, ChevronDown } from "lucide-react";
...
const STATUSES = ["idea", "building", "launched", "paused", "archived"] as const;
type Status = (typeof STATUSES)[number];

const STATUS_META: Record<Status, { label: string; color: string }> = {
  idea: { label: "Idea", color: "#7dbde8" },
  building: { label: "Building", color: "#f59e0b" },
  launched: { label: "Launched", color: "#34d399" },
...
export const VenturesPage = () => {
  ...
  const { data, refetch, isPending: venturesLoading } = useGetList<Venture>("ventures", {
  ...
  const del = (v: Venture) =>
    remove("ventures", { id: v.id, previousData: v }, { onSuccess: () => refetch() });

  // Show active statuses first
  const order: Status[] = ["building", "idea", "launched", "paused", "archived"];
      ...
      {addOpen && (
        <AddVentureDialog
          ...
          onAdd={(payload) =>
            create(
              "ventures",
              { data: { ...payload, sales_id: salesId, position: ventures.length } },
              {
                onSuccess: () => {
                  setAddOpen(false);
                  refetch();
                },
                onError: () => notify("Could not add venture", { type: "error" }),
              },
            )
          }
        />
...
const VentureCard = ({
  ...
  return (
    ...
    >
      ...
      >
        <ChevronDown className={showActions ? "size-3.5 rotate-180 transition-transform" : "size-3.5 transition-transform"} />
        Next actions
      </button>
      {showActions && (
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex gap-2">
            ...
            <Input autoFocus placeholder="Venture name" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
          ...
          <Textarea rows={2} placeholder="What is it?" value={description} onChange={(e) => setDescription(e.target.value)} />
            ...
            <Button
              onClick={() =>
                ...
                onAdd({
                  ...
                  url: url.trim() ? (/^https?:\/\//.test(url) ? url.trim() : `https://${url.trim()}`) : null,
